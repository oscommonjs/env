...
class Object(object):
    ...
    def download_url(self, timeout=60, name=None):
        ...
        if "local" in self.driver.name.lower():
            ...
            #expires = str(datetime.datetime.now()
            #           + datetime.timedelta(seconds=timeout))
            expires = int(time.time()+ timeout)
            if 'azure' in driver_name:
                policy_name="fuck123"
                blob_service = BlockBlobService(account_name=self.driver.key, account_key=base64.b64encode(self.driver.secret))
                # Create the container, if it does not already exist
                #blob_service.create_container(self.container.name)
                # Create a new policy that expires after a week
                access_policy = AccessPolicy(permission=ContainerPermissions.READ + ContainerPermissions.LIST, expiry=datetime.datetime.utcnow() + datetime.timedelta(seconds=timeout))
                # Get the existing identifiers (policies) for the container
                identifiers = blob_service.get_container_acl(self.container.name)
                ...
                blob_service.set_container_acl(self.container.name,identifiers)
                # Generate a new Shared Access Signature token using the policy (by name)
                sas_token = blob_service.generate_container_shared_access_signature(self.container.name,id=policy_name)
                #blob_url = "https://{storage_account_name}.blob.core.windows.net/{bucket}/{file_path}?{sas_token}"
                return "%s?%s" % (self.secure_url, sas_token)
